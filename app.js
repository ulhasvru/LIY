// =====================================================
// BRAINBURST ACADEMY — Merged App.js
// Auth + localStorage + Quiz Engine + Dashboard
// =====================================================
...
const SUBJ_COLORS = {
  math:     { bar:'#FF6B6B', pill:'#C0392B' },
  evs:      { bar:'#06D6A0', pill:'#048060' },
  english:  { bar:'#4CC9F0', pill:'#2E7A99' },
  hindi:    { bar:'#FFD93D', pill:'#A07A00' },
  kannada:  { bar:'#C77DFF', pill:'#7B3AAA' },
  computer: { bar:'#4CC9F0', pill:'#2E7A99' }
};
...
// =====================================================
const LS_USERS   = 'bb_users_v2';
const LS_SESSION = 'bb_session_v2';
...
function persistUserUpdate(updated) {
...
const S = {
  // auth
  user: null,          // current user object from localStorage
  // quiz runtime
  subjectId: null,
  questions: [], qIdx: 0, qScore: 0, correct: 0,
  // accumulated this session (in case user doesn't reload)
  get name()       { return S.user ? S.user.firstName || S.user.name : 'Explorer'; },
  get totalScore() { return S.user ? S.user.totalScore : 0; },
  get badges()     { return S.user ? S.user.badges : []; },
  get subjScores() { return S.user ? S.user.subjScores : {}; }
};
...
function buildAvatarPicker() {
  ...
  AVATARS.forEach(av => {
    ...
    d.className = 'avatar-opt' + (av === selectedAvatar ? ' selected' : '');
    d.textContent = av;
    d.onclick = () => {
      selectedAvatar = av;
...
function doLogin() {
  ...
  if (!identifier || !pass) return showAuthMsg('Please fill in all fields!');

  const user = getUserByEmail(identifier) || getUserByUsername(identifier);
  if (!user)                   return showAuthMsg('Account not found. Please sign up first!');
  if (user.pass !== btoa(pass)) return showAuthMsg('Incorrect password. Try again!');
...
function doSignup() {
  ...
  if (!fname || !lname || !email || !pass || !pass2)
    return showAuthMsg('Please fill in all fields!');
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
  ...
  if (users.find(u => u.email === email))
    return showAuthMsg('An account with this email already exists!');

  const newUser = {
    email,
    username: (fname + lname).toLowerCase().replace(/\s+/g,''),
    name: fname + ' ' + lname,
    firstName: fname,
  ...
  showAuthMsg('Account created! Welcome to BrainBurst! 🎉', 'success');
  setTimeout(() => loadUserSession(newUser), 900);
}
...
// =====================================================
// SESSION LOAD — after login or on page load
// =====================================================
function loadUserSession(user) {
  S.user = user;
  saveSession(user);
  // Populate welcome screen
  document.getElementById('welcome-avatar-display').textContent = user.avatar || '🦁';
  document.getElementById('welcome-display-name').textContent   = '🎉 Welcome back, ' + user.firstName + '!';
  // Sync header avatars
  document.querySelectorAll('#hdr-avatar, #hdr-avatar-subj').forEach(el => el.textContent = user.avatar || '🦁');
  showScreen('screen-welcome');
}

function enterApp() {
  const user = S.user;
  document.getElementById('greeting-name').textContent = user.firstName || user.name;
  renderSubjectsGrid();
  syncUI();
  showScreen('screen-home');
  toast(`🦁 Roar! Let's go, ${user.firstName}! Your adventure continues!`);
}
...
function syncUI() {
  if (!S.user) return;
  const n  = S.user.firstName || S.user.name || 'Explorer';
  const sc = S.user.totalScore || 0;
  document.querySelectorAll('#header-name').forEach(e => e.textContent = n);
  document.querySelectorAll('#header-score,#quiz-hdr-score,#bdg-score,#dash-hdr-score').forEach(e => e.textContent = sc);
  document.querySelectorAll('.dyn-score').forEach(e => e.textContent = sc);
  document.querySelectorAll('#hdr-avatar, #hdr-avatar-subj').forEach(e => e.textContent = S.user.avatar || '🦁');
}
...
function renderQ() {
  ...
  document.getElementById('quiz-hdr-score').textContent = S.user.totalScore;
...
function pick(idx) {
  ...
  if (idx === q.ans) {
    ...
    S.qScore += pts;
    S.user.totalScore = (S.user.totalScore || 0) + pts;
    S.correct++;
    document.getElementById('q-live').textContent = S.qScore;
    document.getElementById('quiz-hdr-score').textContent = S.user.totalScore;
    syncUI();
...
// =====================================================
// RESULTS — with localStorage persistence
// =====================================================
function showResults() {
  const tot = S.questions.length;
  const pct = Math.round((S.correct / tot) * 100);

  // Update best score for subject (keep best)
  const prevBest = S.user.subjScores[S.subjectId];
  if (prevBest === undefined || pct > prevBest) {
    S.user.subjScores[S.subjectId] = pct;
  }

  // Record quiz history entry
  S.user.quizHistory = S.user.quizHistory || [];
  S.user.quizHistory.push({
    subjectId: S.subjectId,
    subjectName: SUBJECTS.find(s => s.id === S.subjectId).name,
    subjectIcon: SUBJECTS.find(s => s.id === S.subjectId).icon,
    correct: S.correct,
    total: tot,
    pct,
    pts: S.qScore,
  ...
  // Badge computation
  const prevBadges = [...(S.user.badges || [])];
  const newBadges = [];
  ALL_BADGES.forEach(b => {
    if (S.user.badges.includes(b.id)) return;
    let earned = false;
    if (b.id === 'all_done') {
      earned = SUBJECTS.every(s => S.user.subjScores[s.id] !== undefined);
    } else if (b.id === 'perfect') {
      earned = pct === 100;
    } else if (b.subjId === S.subjectId && pct >= b.minScore) {
      earned = true;
    }
    if (earned) { S.user.badges.push(b.id); newBadges.push(b); }
  });

  // Persist everything
  persistUserUpdate(S.user);
  renderSubjectsGrid();
  ...
  document.getElementById('res-subj').textContent = S.user.firstName + ' · ' + sv.name + ' Quiz';
  ...
  document.getElementById('rs-total').textContent = S.user.totalScore;
...
function renderBadgesWall() {
  const earned = S.user ? S.user.badges : [];
  ...
  document.getElementById('bdg-score').textContent  = S.user ? S.user.totalScore : 0;
...
function renderDashboard() {
  ...
  const scores  = user.subjScores  || {};
  const earned  = user.badges      || [];

  // Header
  document.getElementById('dash-hdr-score').textContent = user.totalScore || 0;
  document.getElementById('dash-avatar-big').textContent = user.avatar || '🦁';
  document.getElementById('dash-name').textContent = user.firstName || user.name;

  // Join date
  const joined = user.joinedAt ? new Date(user.joinedAt).toLocaleDateString('en-IN', {day:'numeric',month:'long',year:'numeric'}) : 'N/A';
  document.getElementById('dash-joined').textContent = joined;

  // Stat cards
  const avg = history.length ? Math.round(history.reduce((s,q) => s + q.pct, 0) / history.length) : 0;
  document.getElementById('ds-quizzes').textContent = history.length;
  document.getElementById('ds-avg').textContent     = avg + '%';
  document.getElementById('ds-pts').textContent     = user.totalScore || 0;
  document.getElementById('ds-badges').textContent  = earned.length;

  // Subject progress
  const spEl = document.getElementById('dash-subject-progress');
  spEl.innerHTML = SUBJECTS.map(s => {
    const hasDone = scores[s.id] !== undefined;
    const pct = hasDone ? scores[s.id] : 0;
    const col = SUBJ_COLORS[s.id] || { bar:'#CCC', pill:'#999' };
    const pillStyle = pct >= 80 ? `background:${col.bar};box-shadow:0 3px 0 ${col.pill}` : pct >= 50 ? 'background:#FFD93D;box-shadow:0 3px 0 #A07A00;color:#1A1A2E' : 'background:#E5E7EB;box-shadow:0 3px 0 #CBD5E1;color:#4A4A6A';
    return `<div class="dash-subj-row">
      <div class="dsr-icon">${s.icon}</div>
      <div class="dsr-info">
        <div class="dsr-name">${s.name}</div>
        <div class="dsr-bar-bg"><div class="dsr-bar-fill" style="width:${pct}%;background:${col.bar}"></div></div>
      </div>
      ${hasDone
        ? `<div class="dsr-pct" style="${pillStyle}">${pct}%</div>`
        : `<div class="dsr-not-done">Not attempted</div>`}
    </div>`;
  }).join('');

  // Quiz history
  const qhEl = document.getElementById('dash-quiz-history');
  if (!history.length) {
    qhEl.innerHTML = '<div class="dash-empty"><span class="de-emoji">📝</span><p>No quizzes taken yet! Jump into a subject above.</p></div>';
  } else {
    const recentHistory = [...history].reverse().slice(0, 10);
    qhEl.innerHTML = '<div class="dash-quiz-list">' + recentHistory.map(q => {
      const d = new Date(q.date).toLocaleDateString('en-IN', {day:'numeric',month:'short',year:'numeric'});
      const pctColor = q.pct >= 80 ? '#06D6A0' : q.pct >= 50 ? '#FFD93D' : '#FF6B6B';
      const pctShadow = q.pct >= 80 ? '#048060' : q.pct >= 50 ? '#A07A00' : '#C0392B';
      const pctTextColor = q.pct >= 50 ? 'white' : 'white';
      return `<div class="dq-item">
        <div class="dq-icon">${q.subjectIcon || '📚'}</div>
        <div class="dq-info">
          <div class="dq-name">${q.subjectName} Quiz</div>
          <div class="dq-date">${d} · ${q.correct}/${q.total} correct · +${q.pts} pts</div>
        </div>
        <div class="dq-pct" style="background:${pctColor};box-shadow:0 3px 0 ${pctShadow};color:${pctTextColor}">${q.pct}%</div>
      </div>`;
    }).join('') + '</div>';
  }

  // Earned badges showcase
  const dbEl = document.getElementById('dash-badges-row');
  const earnedBadges = ALL_BADGES.filter(b => earned.includes(b.id));
  if (!earnedBadges.length) {
    dbEl.innerHTML = '<div class="dash-empty"><span class="de-emoji">🔒</span><p>Complete quizzes to unlock badges!</p></div>';
  } else {
    dbEl.innerHTML = earnedBadges.map((b,i) =>
      `<div class="dash-badge-pill" style="animation-delay:${i*0.06}s">
        <span class="dbp-icon">${b.icon}</span>${b.name}
      </div>`
    ).join('');
  }
}
